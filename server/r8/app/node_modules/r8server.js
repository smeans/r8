...
class R8Environment {
    ...
    get fileRoot() {
        const root = path.join(config.org_file_store_root, this.organization.id);
...
class R8SqliteTable extends R8Table {
    ...
    query(where, orderBy, limit) {
        ...
        const getOperator = (whereValue) => {
            return typeof whereValue === 'object' ? whereValue.operator : '==';
        }

        const getValue = (whereValue) => {
            return (typeof whereValue === 'object' ? whereValue.value : whereValue);
        }

        const getParmExp = (keyTermName) => {
            const keyTerm = this.term.package.getTerm(keyTermName);
            console.assert(keyTerm);

            return keyTerm ? `cast(? as ${R8SqliteTable.R8_TO_SQLITE_TYPES[keyTerm.dataType]})` : '?';
        }

        const getOrderByExp = (keyTermName) => {
            const keyTerm = this.term.package.getTerm(keyTermName);
            console.assert(keyTerm);

            return keyTerm ? `cast(${keyTerm.name} as ${R8SqliteTable.R8_TO_SQLITE_TYPES[keyTerm.dataType]})` : keyTerm.name;
        }
        ...
        if (typeof where == 'object') {
            ...
            sql += ` where ${cols.map(col => `"${col}" ${getOperator(where[col])} ${getParmExp(col)}`).join(' and ')}`;
        } else {
            where = {};
        }

        if (orderBy !== undefined && orderBy.length) {
            sql += ' order by ' + orderBy.map(v => {
                if (Array.isArray(v)) {
                    return getOrderByExp(v[0]) + ' ' + v.slice(1).join(' ');
                }

                return getOrderByExp(v);
            }).join(', ');
        }
        ...
        try {
            ...
            return stmt.iterate(Object.values(where).map(value => getValue(value)));
        } catch (e) {
            console.warn(e);
...
class R8SqliteTableProvider extends R8TableProvider {
    ...
    lookupTerm(term) {
        ...
        if (!table) {
            console.warn(`bad or missing R8Table: ${term.name}`);
        ...
        term.keyTermNames.forEach(key => {
            const keyTerm = term.package.getTerm(key);

            switch (term.getKeyTermMatchType(key)) {
                case 'Exact': {
                    where[key] = keyTerm.value;
                } break;

                case 'ClosestLowValue': {
                    where[key] = {
                        operator: '<=',
                        value: keyTerm.value
                    };

                    orderBy.push([keyTerm.name, 'DESC']);

                    limit = 1;
                } break;

                case 'ClosestHighValue': {
                    where[key] = {
                        operator: '>=',
                        value: keyTerm.value
                    };

                    orderBy.push(keyTerm.name);
        ...
        return table.query(where, orderBy, limit);
    }
    ...
    getPackageDb(pkg) {
        ...
        if (!(this._packageDbs[packageId])) {
            this._packageDbs[packageId] = new Database(path.join(this.environment.fileRoot, `package-${packageId}.sqlite3`), {
                verbose: console.debug
            });
            this._packageDbs[packageId].serialize();
        }

        return this._packageDbs[packageId];
    }
}
