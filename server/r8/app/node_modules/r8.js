...
class R8ExpressionTerm extends R8Term {
    ...
    get value() {
        const value = this._evaluateExpression();

        this.package.evalContext && this.package.evalContext.setTermValue(this, value);
        ...
        this._expression.forEach(token => {
            switch (token.type) {
                case 'name': {
                    ...
                    if (value === undefined) {
                        this.package.evalContext.warn(this, `term ${term.name} value is undefined`);
                    }
                    stack.push(value);
                } break;
...
class R8InputTerm extends R8Term {
    ...
    getChoices() {
        ...
        this.package.getReferencingTerms(this).forEach((term) => {
            console.log('getting choices', term.name);
...
class R8TableTerm extends R8Term {
    ...
    get value() {
        ...
        switch (rows.length) {
            case 0: {
                this.package.evalContext.warn(this, `no lookup found: keys ${Array.from(this.keyTermNames).join(', ')}`);
            ...
            default: {
                this.package.evalContext.warn(this, `multiple matching rows: keys ${Array.from(this.keyTermNames).join(', ')}`);
    ...
    deleteKeyTerm(termName) {
        console.log('deleting key term', termName);
