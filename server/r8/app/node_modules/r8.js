...
class R8Linter {
    ...
    validateIdentifier(identifier) {
        if (R8Linter.INTERNAL_ID.test(identifier)) {
            return true;
        }
...
class R8ExpressionTerm extends R8Term {
    ...
    set expressionSource(newExpressionSource) {
        ...
        // !!!TBD!!! wsm - we could check the resulting expression stack for
        // validity here before saving it and throw an exception. For example:
        // "Foo + Bar Baz Fubar" will save correctly
        ...
        this._expression.forEach(token => {
            switch (token.type) {
                case 'name': {
                    ...
                    if (valueType.value === undefined || valueType.value === null) {
                        this.package.evalContext && this.package.evalContext.warn(this, `${term.termTypeName} term ${term.name}: value is undefined`);
                    }
...
class R8Package {
    ...
    static fromJson(json, linter) {
        const pkg = new R8Package(linter);
