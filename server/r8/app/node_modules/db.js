...
class Organization extends R8Document {
    ...
    async getPackageList() {
        const packages = this.collection.find({objectType: 'Package'});
    ...
    async createPackage(packageName) {
        ...
        const doc = this.collection.insertOne({
    ...
    async getPackage(packageId) {
        ...
        try {
            doc = await this.collection.findOne({_id: new ObjectId(packageId)});
        } catch {
            doc = await this.collection.findOne({packageName: packageId});
        }
    ...
    async savePackage(pkg, versionInfo) {
        ...
        const doc = await this.collection.findOne({_id: new ObjectId(pkg._id)});

        if (doc) {
            delete doc._id;
            doc.archiveTs = new Date();
            await this.collection.insertOne(doc);
        }

        const json = pkg.json;
        versionInfo.saveTs = new Date();
        json.versionInfo = versionInfo;

        return await this.collection.replaceOne({_id: pkg._id}, json);
    }
