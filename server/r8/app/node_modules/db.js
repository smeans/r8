...
class Organization extends R8Document {
    ...
    async createPackage(product, effectiveDate, versionInfo) {
        ...
        this.savePackage(pkg, versionInfo);
    ...
    async _getPackageDoc(packageId) {
        try {
            return this.collection.findOne({
                objectType: 'Package',
                packageId: packageId, archiveTs: {$exists: false}});
        } catch {
            return null;
        }
    }
    ...
    async deploy(deployInfo, packageId) {
        console.assert(this.environments.indexOf(this.currentEnvironment) < this.environments.length-1);
        ...
        const nextEnv = this.environments[this.environments.indexOf(this.currentEnvironment)+1];

        console.debug(`${product.name}: deploying package ${pkg.packageId} from ${this.currentEnvironment} to ${nextEnv}`);
        ...
        if (pkg.versionId.equals(status[nextEnv].versionId)) {
            return;
        }
