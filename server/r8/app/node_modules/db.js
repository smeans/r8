...
class Organization extends R8Document {
    ...
    async createPackage(packageName) {
        const doc = this.collection.insertOne({
            ...
            objectType: 'Package',
            lastModifyTs: new Date(),
            terms: {}
        });

        return doc ? R8Package.fromJson(doc) : null;
    }

    async getPackage(packageId) {
        const doc = await this.collection.findOne({_id: new ObjectId(packageId)});
        return doc ? R8Package.fromJson(doc) : null;
    }

    async savePackage(pkg) {
        console.debug('savePackage', pkg.json);
        return await this.collection.replaceOne({_id: pkg._id}, pkg.json);
    }
}
