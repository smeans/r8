...
const db = camo.connect(config.mongodb.url);
...
class Organization extends R8Document {
    ...
    static getUniqueHostName() {
        const epoch = new Date(2022, 0, 1);

        return 'org-' + parseInt((Date.now()-epoch).toString().split('').reverse().join('')).toString(36);
    }
    ...
    constructor() {
        ...
        this.apiDomain = String;
    }

    async preSave() {
        if (!this.apiDomain) {
            this.apiDomain = Organization.getUniqueHostName();
        }
    }
    ...
    get collection() {
        return client.db('r8').collection(`org-${this.id}`);
    }
    ...
    async findProductByName(name, state) {
        ...
        return await await Product.findOne(query);
    }
    ...
    async createProduct(productName, description, states) {
        if (!/^[\w\d\-]+$/.test(productName)) {
            throw RangeError('invalid product name: product names can only contain letters, numbers, and the hyphen (-) character');
        }
    ...
    async deleteProduct(productId) {
        ...
        console.debug('deleteProduct', product);

        if (!product) {
            throw Error(`${productId} not found or already deleted`);
        }

        const packageLists = await this.getPackageLists(productId);
        const prodEnv = this.environments[this.environments.length-1];

        if (packageLists[prodEnv].length) {
            throw Error(`cannot delete product ${product.name}: versions have been deployed to the ${prodEnv} environment`);
        }
    ...
    async _getPackageDoc(packageId) {
        try {
            if (this.environments.indexOf(this.currentEnvironment) == 0) {
            ...
            const status = await this.getPackageStatus(packageId);
            const versionId = status[this.currentEnvironment].versionId;

            if (!versionId) {
                return null;
            }

            return this.collection.findOne(versionId);
        } catch {
            return null;
        }
    }
    ...
    async getEffectivePackage(productId, effectiveDate) {
        console.assert(effectiveDate instanceof Date);
        ...
        if (this.environments.indexOf(this.currentEnvironment) > 0) {
            query.objectType = 'DeployInfo';
            query.environment = this.currentEnvironment;
            query.replacedById = {'$exists': false};

            const doc = await this.collection.find(query, {versionId: 1})
                    .sort({effectiveDate: 1}).limit(1).next();

            if (doc) {

                const pkg = await this.getPackageVersion(doc.versionId);
                if (pkg) {
                    pkg.environment = this.currentEnvironmentObject;
                }
            ...
            query.objectType = 'Package';
            query.archiveTs = {$exists: false};

            const doc = await this.collection.find(query, {packageId: 1})
                    .sort({effectiveDate: 1}).limit(1).next();

            return doc && this.getPackage(doc.packageId);
        }
    }
    ...
    async deploy(packageId, deployInfo, fromEnvironment) {
        ...
        if (environmentIndex < 0) {
            throw Error(`deploy: bad from environment ${fromEnvironment}`);
        }

        if (environmentIndex >= this.environments.length-1) {
            throw Error(`no higher environment to deploy to from ${fromEnvironment}`);
        }
        ...
        if (issues && issues.length) {
            throw Error(`unable to deploy due to the following package issues:\n\t${issues.join('\n\t')}`);
        }
    ...
    async getPackageStatus(packageId) {
        const pkgDoc = await this.collection.findOne({
            objectType: 'Package',
            packageId: packageId,
            archiveTs: {$exists: false}
        });
        ...
        const resp = {
                [this.environments[0]]: {
                    versionId: pkgDoc._id,
                    versionInfo: pkgDoc.versionInfo
                }
        };
...
class Product extends R8Document {
    ...
    async preSave() {
        ...
        const statesSet = new Set(this.states);

        for await (const product of duplicates) {
            let overlap = product.states.filter(state => statesSet.has(state));
            if (overlap.length) {
                throw Error(`unable to save ${product.name}: an existing product named ${product.name} is defined for state(s): ${overlap.join(',')}`);
            }
        }
    }    
}
